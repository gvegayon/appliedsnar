...
For more on control parameters, see @Morris2008. The next model consists of a simpler version. Excluding the mutual term leads to an ERGM with no Markov terms, meaning that all the sufficient statistics feature one tie at a time. The mutual term included two ties ($i \to j$ and $j \to i$). Models like this reduce to a logistic regression, which is why the fitting process avoids using MCMC:
