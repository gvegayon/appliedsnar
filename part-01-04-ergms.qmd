...
@lusherExponentialRandomGraph2013;@admiraal2006;@Snijders2002;@Wang2009 provides details on the algorithm used by PNet (the same as the one used in `RSiena`), and @lusherExponentialRandomGraph2013 provides a short discussion on the differences between `ergm` and `PNet`. 
...
Coloring by `hispanic`, we can see that nodes seem to be clustered by this attribute. Visual inspection in network science can be very useful, but, in order to make more formal statements, we need to use statistical models; this is where ERGMs shine.

## Estimating ERGMs

Although there is no silver bullet for estimating these models, the following workflow has worked for me in the past:
...
::: {.callout-note title="Previous version of the book"}
In a previous version of the book we used the `bd` constraint to limit the maximum number of outgoing ties to 19. Although this is a reasonable constraint (as the sample may have been restricted by construction), the current version of the ERGM package returns a warning and shows the GOF statistics with switched signs (negative AIC and BIC, and positive Loglikelihood).
:::
...
For more on control parameters, see @Morris2008. The next model consists of a simpler version. Excluding the mutual term leads to an ERGM with no Markov terms, meaning that all the sufficient statistics are functions of individual edges rather than configurations involving multiple edges. The mutual term, by contrast, involves pairs of ties ($i \to j$ and $j \to i$), which is an example of a Markov term. Models with only non-Markov terms reduce to a logistic regression, which is why the fitting process avoids using MCMC:
...
Finally, we can try incorporating a more complex term, for example, the geometrically weighted edge-wise shared partners (gwesp) term. This term is akin to triadic closure, but it is easier to estimate. Moreover, to ease the estimation process, it is recommended to add the geometrically weighted dyad-wise shared partners (gwdsp) term too. Both terms include a decay parameter; for simplicity, both will be fixed and set to be 0.5 (values between 0.25 and 1.5 are common). For more on this, see @hunterCurvedExponentialFamily2007.
...
Before we go to the next section, let's make some comments on what we see in the table:

1. Of the three models, the last one is the best in terms of smallest AIC and BIC, and largest Loglikelihood.

2. All the nodematch terms are significant and positive, which means that we have homophily by `hispanic`, `female1`, and `eversmk1`.

3. The `mutual` term is significant, positive, and large (~4). This indicates a strong tendency for individuals to befriend each other.

4. The `gwesp` term is positive and significant. The interpretation of this term is not always simple. Generally, we say that a positive gwesp term means that there is a tendency for triadic closure. 
...
Reporting ERGM results can be done smoothly using a combination of two R packages: [`texreg`](https://cran.r-project.org/package=texreg){target="_blank"} and [`tabulergm`](https://github.com/gvegayon/tabulergm){target="_blank"}. The `texreg` package, which we used in the previous chapter, provides an easy way to create regression tables featuring multiple models at the same time. The `tabulergm` R package, on the other hand, consists of a tool to describe your network models. Let's see them in action; we will start with `texreg`:

::: {.callout-important}
The `tabulergm` package is a new package that was recently submitted to CRAN. While it is reviewed by CRAN, you can get a copy of the package at <https://github.com/gvegayon/tabulergm>
:::

```{r}
#| label: tabulation-texreg
#| results: asis
texreg::knitreg(ans2, single.row = TRUE)
```

We can directly pass the model object, but the formula would provide a summary table including the mathemathical and graphical representation (a drawing) of the terms, as well as a description pulled from the ERGM package:

```{r}
#| label: tabulation-tabulergm
#| results: asis
tabulergm::tabulergm_table(
  formula(ans2),
  format = "markdown"
  )
```
...
In particular, the log-odds of the $ij$ tie occurring conditional on the rest of
...
\end{align*}
\noindent Therefore, the conditional probability of the tie $y_{ij}$ occurring can be written as:
...
The challenge of analyzing networks is their interdependent nature. Nonetheless, in the absence of such interdependence, ERGMs are equivalent to logistic regression. Conceptually, if all the statistics included in the model do not involve two or more dyads, then the model is dyadic independent.

Mathematically, to see this, it suffices to show that the ERGM probability can be written as the product of each dyads' individual probabilities.
...
Where $\s{}_{ij}$ is a function such that $\s{\mathbf{y}} = \sum_{ij}{\s{\mathbf{y}}_{ij}}$. We now need to deal with the normalizing constant. The key lies in the fact that we can swap the sum and the product operators. In the case of the dyadic independent model, the normalizing constant can be written as:

$$
\sum_{y\in\mathcal{Y}}\exp{\transpose{\theta}\s{\mathbf{y}}} = %
  \sum_{y\in\mathcal{Y}}\prod_{ij}\exp{\transpose{\theta}\s{\mathbf{y}}_{ij}} = %
  \prod_{ij}\sum_{y\in\{0,1\}}\exp{\transpose{\theta}\s{\mathbf{y}_{ij}}}
$$

This can only be achieved with dyadic independence. Otherwise, this arrangement cannot be done. Now, we can write the ERGM model as follows:

$$
\prod_{ij}\left[\frac{%
\exp{\transpose{\theta}\s{\mathbf{y}}_{ij}}%
}{%
\sum_{y'\in\{0,1\}}\exp{\transpose{\theta}\s{\mathbf{y}}_{ij}}%
}\right] = %
\prod_{ij}\left[\frac{1}{%
1 + \exp{-\transpose{\theta}\Delta\chng{ij}}}%
\right]
$$


Where the last equality comes from dividing each by the $ij$-th numerator. Since at least one of the two elements of the $\sum_{y'\in\{0,1\}}$ matches the observed $y_{ij}$, one reduces to one, and the other to the change statistic.
