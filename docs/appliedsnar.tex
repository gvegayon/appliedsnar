...
\begin{verbatim}
## + 1/2638 edge from 2e0c657 (vertex names):
## [1] 1110111->1110111
\end{verbatim}
...
\begin{verbatim}
## [[1]]
## + 1/4 vertex, from 5c09c56:
## [1] 2
## 
## [[2]]
## + 2/4 vertices, from 5c09c56:
## [1] 1 3
## 
## [[3]]
## + 3/4 vertices, from 5c09c56:
## [1] 2 4 4
## 
## [[4]]
## + 2/4 vertices, from 5c09c56:
...
\NormalTok{n_a       <-}\StringTok{ }\DecValTok{4}   \CommentTok{# Number of alters}
\NormalTok{sizes     <-}\StringTok{     }\CommentTok{# Sizes to try}
\StringTok{  }\KeywordTok{seq}\NormalTok{(}\DataTypeTok{from =} \DecValTok{100}\NormalTok{, }\DataTypeTok{to =} \DecValTok{250}\NormalTok{, }\DataTypeTok{by =} \DecValTok{25}\NormalTok{)}

\CommentTok{# Assumptions}
\NormalTok{odds_h_}\DecValTok{1}\NormalTok{  <-}\StringTok{ }\FloatTok{2.0} \CommentTok{# Odds of Increase/}
...
\NormalTok{  res_alter <-}\StringTok{ }\KeywordTok{tryCatch}\NormalTok{(}\KeywordTok{glm}\NormalTok{(y_alter }\OperatorTok{~}\StringTok{ }\NormalTok{tr_alter, }\DataTypeTok{family =} \KeywordTok{binomial}\NormalTok{(}\StringTok{"logit"}\NormalTok{)), }\DataTypeTok{error =} \ControlFlowTok{function}\NormalTok{(e) e)}
  ...
  \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{inherits}\NormalTok{(res_ego, }\StringTok{"error"}\NormalTok{) }\OperatorTok{|}\StringTok{ }\KeywordTok{inherits}\NormalTok{(res_alter, }\StringTok{"error"}\NormalTok{))}
    \KeywordTok{return}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\DataTypeTok{ego =}  \OtherTok{NA}\NormalTok{, }\DataTypeTok{alter =} \OtherTok{NA}\NormalTok{))}
  
  \CommentTok{# Step 5: Reject?}
  \KeywordTok{c}\NormalTok{(}
    \DataTypeTok{ego   =} \KeywordTok{summary}\NormalTok{(res_ego)}\OperatorTok{$}\NormalTok{coefficients[}\StringTok{"tr"}\NormalTok{, }\StringTok{"Pr(>|z|)"}\NormalTok{] }\OperatorTok{<}\StringTok{ }\NormalTok{alpha,}
    \DataTypeTok{alter =} \KeywordTok{summary}\NormalTok{(res_alter)}\OperatorTok{$}\NormalTok{coefficients[}\StringTok{"tr_alter"}\NormalTok{, }\StringTok{"Pr(>|z|)"}\NormalTok{] }\OperatorTok{<}\StringTok{ }\NormalTok{alpha}
\NormalTok{  )}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{beta_pars <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{4}\NormalTok{, }\DecValTok{6}\NormalTok{)}
...
\NormalTok{  y_ego_}\DecValTok{1}\NormalTok{   <-}\StringTok{ }\KeywordTok{runif}\NormalTok{(n) }\OperatorTok{<}\StringTok{ }\KeywordTok{plogis}\NormalTok{(}\KeywordTok{qlogis}\NormalTok{(y_ego_star) }\OperatorTok{+}\StringTok{ }\KeywordTok{log}\NormalTok{(odds_h_}\DecValTok{1}\NormalTok{))}
\NormalTok{  tr_alter  <-}\StringTok{ }\KeywordTok{as.integer}\NormalTok{(}\KeywordTok{rep}\NormalTok{(y_ego_}\DecValTok{1}\NormalTok{, n_a))}
\NormalTok{  y_alter_}\DecValTok{1}\NormalTok{ <-}\StringTok{ }\KeywordTok{runif}\NormalTok{(n }\OperatorTok{*}\StringTok{ }\NormalTok{n_a) }\OperatorTok{<}\StringTok{ }\KeywordTok{plogis}\NormalTok{(}\KeywordTok{qlogis}\NormalTok{(y_alter_star) }\OperatorTok{+}\StringTok{ }\KeywordTok{log}\NormalTok{(odds_h_}\DecValTok{1}\NormalTok{) }\OperatorTok{*}\StringTok{ }\NormalTok{tr_alter) }\CommentTok{# So only if ego did something}

  \CommentTok{# Step 4: Computing test statistic}
\NormalTok{  y_ego_}\DecValTok{0}\NormalTok{ <-}\StringTok{ }\KeywordTok{as.integer}\NormalTok{(y_ego_}\DecValTok{0}\NormalTok{)}
\NormalTok{  y_ego_}\DecValTok{1}\NormalTok{ <-}\StringTok{ }\KeywordTok{as.integer}\NormalTok{(y_ego_}\DecValTok{1}\NormalTok{)}
\NormalTok{  y_alter_}\DecValTok{0}\NormalTok{ <-}\StringTok{ }\KeywordTok{as.integer}\NormalTok{(y_alter_}\DecValTok{0}\NormalTok{)}
\NormalTok{  y_alter_}\DecValTok{1}\NormalTok{ <-}\StringTok{ }\KeywordTok{as.integer}\NormalTok{(y_alter_}\DecValTok{1}\NormalTok{)}

\NormalTok{  d <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}
    \DataTypeTok{y  =} \KeywordTok{c}\NormalTok{(y_ego_}\DecValTok{0}\NormalTok{, y_ego_}\DecValTok{1}\NormalTok{),}
    \DataTypeTok{tr =} \KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\DecValTok{0}\NormalTok{, n), }\KeywordTok{rep}\NormalTok{(}\DecValTok{1}\NormalTok{, n)),}
    \DataTypeTok{g  =} \KeywordTok{c}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{n, }\DecValTok{1}\OperatorTok{:}\NormalTok{n)}
\NormalTok{  )}

\NormalTok{  res_ego   <-}\StringTok{ }\KeywordTok{tryCatch}\NormalTok{(}
    \KeywordTok{clogit}\NormalTok{(y }\OperatorTok{~}\StringTok{ }\NormalTok{tr }\OperatorTok{+}\StringTok{ }\KeywordTok{strata}\NormalTok{(g), }\DataTypeTok{data =}\NormalTok{ d, }\DataTypeTok{method =} \StringTok{"exact"}\NormalTok{),}
    \DataTypeTok{error =} \ControlFlowTok{function}\NormalTok{(e) e}
\NormalTok{    )}

\NormalTok{  d <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}
    \DataTypeTok{y  =} \KeywordTok{c}\NormalTok{(y_alter_}\DecValTok{0}\NormalTok{, y_alter_}\DecValTok{1}\NormalTok{),}
    \DataTypeTok{tr =} \KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\DecValTok{0}\NormalTok{, n }\OperatorTok{*}\StringTok{ }\NormalTok{n_a), tr_alter),}
    \DataTypeTok{g  =} \KeywordTok{c}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{(n }\OperatorTok{*}\StringTok{ }\NormalTok{n_a), }\DecValTok{1}\OperatorTok{:}\NormalTok{(n }\OperatorTok{*}\StringTok{ }\NormalTok{n_a))}
\NormalTok{  )}

\NormalTok{  res_alter <-}\StringTok{ }\KeywordTok{tryCatch}\NormalTok{(}
    \KeywordTok{clogit}\NormalTok{(y }\OperatorTok{~}\StringTok{ }\NormalTok{tr }\OperatorTok{+}\StringTok{ }\KeywordTok{strata}\NormalTok{(g), }\DataTypeTok{data =}\NormalTok{ d, }\DataTypeTok{method =} \StringTok{"exact"}\NormalTok{),}
    \DataTypeTok{error =} \ControlFlowTok{function}\NormalTok{(e) e}
\NormalTok{    )}
  ...
  \CommentTok{# Step 5: Reject?}
  \KeywordTok{c}\NormalTok{(}
    \CommentTok{# ego        = res_ego$p.value < alpha,}
    \DataTypeTok{ego        =} \KeywordTok{summary}\NormalTok{(res_ego)}\OperatorTok{$}\NormalTok{coefficients[}\StringTok{"tr"}\NormalTok{, }\StringTok{"Pr(>|z|)"}\NormalTok{] }\OperatorTok{<}\StringTok{ }\NormalTok{alpha,}
    \DataTypeTok{alter      =} \KeywordTok{summary}\NormalTok{(res_alter)}\OperatorTok{$}\NormalTok{coefficients[}\StringTok{"tr"}\NormalTok{, }\StringTok{"Pr(>|z|)"}\NormalTok{] }\OperatorTok{<}\StringTok{ }\NormalTok{alpha,}
    \DataTypeTok{ego_test   =} \KeywordTok{coef}\NormalTok{(res_ego),}
    \DataTypeTok{alter_glm  =} \KeywordTok{coef}\NormalTok{(res_alter)}
\NormalTok{  )}
...
\ControlFlowTok{for}\NormalTok{ (s }\ControlFlowTok{in}\NormalTok{ sizes) \{}
  ...
  \CommentTok{# Run the simulation for size s}
\NormalTok{  simres <-}\StringTok{ }\KeywordTok{rowMeans}\NormalTok{(}
    \KeywordTok{replicate}\NormalTok{(n_sims, }\KeywordTok{sim_data_prepost}\NormalTok{(s)),}
...
\NormalTok{spowerd <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(}
  \KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{size =}\NormalTok{ sizes, }\DataTypeTok{power =}\NormalTok{ spower[,}\StringTok{"ego"}\NormalTok{], }\DataTypeTok{type =}  \StringTok{"ego"}\NormalTok{),}
  \KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{size =}\NormalTok{ sizes, }\DataTypeTok{power =}\NormalTok{ spower[,}\StringTok{"alter"}\NormalTok{], }\DataTypeTok{type =}  \StringTok{"alter"}\NormalTok{)}
\NormalTok{)}

\NormalTok{spowerd }\OperatorTok{|}\ErrorTok{>}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ size, }\DataTypeTok{y =}\NormalTok{ power, }\DataTypeTok{colour =}\NormalTok{ type)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_smooth}\NormalTok{(}\DataTypeTok{method =} \StringTok{"loess"}\NormalTok{, }\DataTypeTok{formula =}\NormalTok{ y }\OperatorTok{~}\StringTok{ }\NormalTok{x) }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"Number of Egos"}\NormalTok{, }\DataTypeTok{y =} \StringTok{"Approx. Power"}\NormalTok{, }\DataTypeTok{colour =} \StringTok{"Node type"}\NormalTok{) }\OperatorTok{+}
...
\NormalTok{power_model <-}\StringTok{ }\KeywordTok{glm}\NormalTok{(}
\NormalTok{  size }\OperatorTok{~}\StringTok{ }\NormalTok{power }\OperatorTok{+}\StringTok{ }\KeywordTok{I}\NormalTok{(power}\OperatorTok{^}\DecValTok{2}\NormalTok{),}
  \DataTypeTok{data =}\NormalTok{ spowerd, }\DataTypeTok{family =} \KeywordTok{gaussian}\NormalTok{(), }\DataTypeTok{subset =}\NormalTok{ type }\OperatorTok{==}\StringTok{ "alter"}
\NormalTok{)}
