...
\NormalTok{  res_ego   <-}\StringTok{ }\KeywordTok{tryCatch}\NormalTok{(}
    \KeywordTok{t.test}\NormalTok{(y_ego_}\DecValTok{0}\NormalTok{, y_ego_}\DecValTok{1}\NormalTok{, }\DataTypeTok{paired =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{alternative=}\StringTok{"two.sided"}\NormalTok{),}
    \DataTypeTok{error =} \ControlFlowTok{function}\NormalTok{(e) e}
\NormalTok{    )}

\NormalTok{  res_alter <-}\StringTok{ }\KeywordTok{tryCatch}\NormalTok{(}
    \KeywordTok{glm}\NormalTok{(y_alter_}\DecValTok{1} \OperatorTok{~}\StringTok{ }\NormalTok{y_alter_}\DecValTok{0} \OperatorTok{+}\StringTok{ }\NormalTok{tr_alter, }\DataTypeTok{family =} \KeywordTok{binomial}\NormalTok{(}\StringTok{"logit"}\NormalTok{)),}
    \DataTypeTok{error =} \ControlFlowTok{function}\NormalTok{(e) e}
\NormalTok{    )}
  ...
  \CommentTok{# Step 5: Reject?}
  \KeywordTok{c}\NormalTok{(}
    \DataTypeTok{ego   =}\NormalTok{ res_ego}\OperatorTok{$}\NormalTok{p.value }\OperatorTok{<}\StringTok{ }\NormalTok{alpha,}
    \DataTypeTok{alter =} \KeywordTok{summary}\NormalTok{(res_alter)}\OperatorTok{$}\NormalTok{coefficients[}\StringTok{"tr_alter"}\NormalTok{, }\StringTok{"Pr(>|z|)"}\NormalTok{] }\OperatorTok{<}\StringTok{ }\NormalTok{alpha,}
    \DataTypeTok{mean_ego =} \KeywordTok{mean}\NormalTok{(y_ego_}\DecValTok{0} \OperatorTok{<}\StringTok{ }\NormalTok{y_ego_}\DecValTok{1}\NormalTok{) ,}
    \DataTypeTok{mean_alter =} \KeywordTok{coef}\NormalTok{(res_alter)[}\DecValTok{2}\NormalTok{]}
\NormalTok{  )}
