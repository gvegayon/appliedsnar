...
\def\labelenumi{\arabic{enumi}.}
\item
  Give an initial guess for \(\theta^{(t)}\), and simulate networks with
  that
\item
  With the networks simulated, we can compute the gradient and hessian
  of the log-likelihood, and hence update the parameter
  \(\theta^{(t)}\to\theta^{(t+1)}\)
\item
  If convergence has been reach (depending on the criteria) stop,
  otherwise go to step 1.
\end{enumerate}

For more details see (Snijders
\protect\hyperlink{ref-Snijders2002}{2002}), and (Wang et al.
\protect\hyperlink{ref-Wang2009}{2009}) provides a nice comparison
between the algorithm used in \texttt{ergm} and in \texttt{PNet} (which
is the same as the one used in \texttt{RSiena}).
...
\begin{verbatim}
## + 1/2638 edge from 38739aa (vertex names):
## [1] 1110111->1110111
\end{verbatim}
