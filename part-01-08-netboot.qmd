---
description: "Hypothesis testing in networks with non-parametric methods in R: comparing networks, testing the prevalence of motifs such as transitive triads, and bootstrapping."
---
...
$\text{density}(G) = \frac{1}{n(n-1)}\sum_{ij}A_{ij}$.

1. Since $s(G_i)\sim \text{N}(\mu_i,\sigma_i^2/m_i)$ for $i\in\{1,2\}$, in the case
...
1. One approach that we can test is whether $k \in \text{ConfInt}(s(G_1) - s(G_2))$.
...
$\text{AvgIndeg}(G_1) \sim N(\mu_1, \sigma^2_1/n)$. Thus, taking advantage of
