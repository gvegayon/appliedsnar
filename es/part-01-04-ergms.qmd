...
description: "Modelos Exponenciales de Grafos Aleatorios (ERGMs) en R con el paquete ergm: especificación del modelo, estimación, diagnósticos MCMC, convergencia y bondad de ajuste."
---
...
![Fuente: Hunter et al. (2008)](img/hunter2008.png){fig-alt="Dos diagramas de redes de Hunter et al. (2008): (a) la red samplike, con vínculos dirigidos entre nodos dibujados como círculos, diamantes y triángulos de colores, y (b) la red faux.mesa.high, con nodos coloreados y con formas según el grado escolar, del 7 al 12."}
...
Finalmente, podemos intentar estimar un modelo con términos más complejos, por ejemplo, el *geometrically weighted edge-wise shared partner* (gwesp). Éste término captura el cierre triádico, pero es más fácil de estimar que `triangle`. Más aún, para facilitar el proceso de estimación, podemos incorporar el termino *geometrically weighted dyad-wise shared partner* (gwdsp). Ambos estadísticos incluyen un parámetro de descuento (decay); por el momento, fijaremos ambos parametros en 0.5 (valores entre 0.25 y 1.5 son comunes). Más detalles sobre estos términos en @hunterCurvedExponentialFamily2007.
...
![Un ejemplo de un ERGM terrible (no hay convergencia en absoluto). También, un buen ejemplo de por qué ejecutar múltiples cadenas puede ser útil](img/awful-chains.png){fig-align="center" fig-alt="Diagnósticos MCMC de un ERGM que no converge. Los gráficos de traza de los estadísticos nodematch.eversmk1, gwesp y gwesp.decay muestran cuatro cadenas que se separan en lugar de mezclarse, y las densidades correspondientes muestran cuatro modas separadas que no se traslapan."}
...
\end{align*}
\noindent Por lo tanto, la probabilidad condicional de que el vínculo $y_{ij}$ ocurra puede escribirse como:
