...
As the variable spans up to `r photo_id_ran[2]`, we need to set the last `r nchar(photo_id_ran[2])` units of the variable to store the `photoid`. We will use `dplyr` [@R-dplyr] and the pipe operator `|>` to create this variable, and we will call it... `id`:[^onpipes]
